...
public class PerlinNoiseGenerator
{
    ...
    public static double noise3 (double x, double y, double z)
    {
        ...
        int BA = p[B] + Z;
        int BB = p[B + 1] + Z;
        return lerp(w, lerp(v, lerp(u, grad(p[AA], x, y, z), grad(p[BA],
                x - 1.0D, y, z)), lerp(u, grad(p[AB], x, y - 1.0D, z), grad(
                p[BB], x - 1.0D, y - 1.0D, z))), lerp(v, lerp(u, grad(
                p[AA + 1], x, y, z - 1.0D), grad(p[BA + 1], x - 1.0D, y,
                z - 1.0D)), lerp(u, grad(p[AB + 1], x, y - 1.0D, z - 1.0D),
