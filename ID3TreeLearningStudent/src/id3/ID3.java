...
public class ID3 {
	...
	public TreeNode id3(List<Example> examples, List<Attribute> attributes, Decision defaultDecision){
		...
		//else (see pseudocode)
			...
			Attribute bestA = helpers.chooseAttributeB(attributes, examples);
			
			//tree <-- a new decision tree with root test best
			//Attribute tree = best.get(0);
			
			int bestALoc = attributes.indexOf(bestA);
			Attribute tree = attributes.get(bestALoc);
			
			best.remove(0);
			
			//for each value vi of best do:
			//or
			Set<String> possibleAnswers = bestA.getPossibleAnswers();
			for (String v : possibleAnswers) {
				
				List<Example> newExamps = new ArrayList<Example>();
						
				for (Example e : examples) {
					if (e.getValue(bestA) == v) {
						newExamps.add(e);
					}
				}
